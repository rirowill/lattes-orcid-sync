...
import { generateLattesDocx } from "@/lib/docx-generator";
...
export async function POST(request: Request) {
  ...
  try {
    const docx = await generateLattesDocx(curriculo, orcidId || undefined);
    const fileName = `${slugifyFileName(curriculo.dadosPessoais?.nomeCompleto)}.docx`;
    ...
    const message = error instanceof Error ? error.message : "Falha ao gerar o DOCX.";
